P=[5:5:195];%no of cars in the road
L=201; %length of road (includes the traffic signal occupying unit length)
maxtime=10000*5; %Max time upto which simulation is done
...
while (time<maxtime)
    ...
    while (green<greentime)
        dis_h=[(diff(l)-1),(L-l(end)+l(1)-2)];%distance head %%%changed from l(1,1) to l(1) to avoid confusion
