...
realization = 1;
silon=0.5;%rand(1,realization); %magnitude of heterogeneity in alpha, bta and rt
rr = 0.5;    %limit of random term in the acceleration
gam = 1;    % relative velocity coefficient
...
%        wt{kl,lk}=wt_zr_vl(1:end); %discarding the first 2000 waiting time values as transients   %%%% changed from 10000 to 1
        wt=[wt,wt_zr_vl(1000:end)]; %discarding the first 2000 waiting time values as transients   %%%% changed from 10000 to 1
    end
