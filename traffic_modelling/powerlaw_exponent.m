function alpha = powerlaw_exponent(batch)
...
if(length(pfil.wt{1})>0)
    if(mod(batch-1,5)==0)
        [alpha, xmin, L] = plfit(pfil.wt{1});%fixing the xmin to be 3.0
...
save(['power_law_exponents_batch',num2str(batch),'.mat'],'alpha','xmin','L');
