...
dt = 0.05;    %%% fixed dt or time interval between updates
            ...
            while (green<greentime)
                ...
                for klj=1:p
                    ...
                    if (vel_h(fp==klj)<thresholdspeed)%&&(l(fp==1)>65)
                        if ((tmp(klj)==0) & (abs(acc(fp==klj))>0.0001))
                            tmp(klj)=1;
                            z_s_t(klj)=time+green - min(dt,abs((thresholdspeed-vel_h(fp==klj))/acc(fp==klj)));%%% time at which the velocity drops below the threshold
                        end
                   end
                   
                   if (~(vel_h(fp==klj)<thresholdspeed))
                       if ((tmp(klj)==1) & (abs(acc(fp==klj))>0.0001))
                           tmp(klj)=0;
                           wt_zr_vl=[wt_zr_vl,time+green-z_s_t(klj) - min(dt,abs((thresholdspeed-vel_h(fp==klj))/acc(fp==klj)))];%%% time at which the velocity rises above the threshold
                       end
                   end
                end
            ...
            while (red<redtime)
                ...
                for klj=1:p
                    ...
                    if (vel_h(fp==klj)<thresholdspeed)%&&(l(fp==1)>65)
                        if ((tmp(klj)==0) & (abs(acc(fp==klj))>0.0001))
                            tmp(klj)=1;
                            z_s_t(klj)=time+green+red - min(dt,abs((thresholdspeed-vel_h(fp==klj))/acc(fp==klj)));%%% time at which the velocity drops below the threshold
                        end
                    end
                    ...
                    if (~(vel_h(fp==klj)<thresholdspeed))
                        if ((tmp(klj)==1) & (abs(acc(fp==klj))>0.0001))
                            tmp(klj)=0;
                            wt_zr_vl=[wt_zr_vl,time+green+red-z_s_t(klj) - min(dt,abs((thresholdspeed-vel_h(fp==klj))/acc(fp==klj)))];%%% time at which the velocity rises above the threshold
                        end
                    end
                end
