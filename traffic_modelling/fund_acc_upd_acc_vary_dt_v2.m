...
rr = 0.0;    %limit of random term in the acceleration
...
for kl=1:1%no of realization
    for lk=1:length(P)
        p=P(lk)
        alpha=0.6;%gamrnd(silon,0.6/silon,1,p);%rand(1,P);%0.2;  %constant1 for friction coefficent (vehicle to the road);  %%NOV17
        bta=0.2;%gamrnd(silon,0.2/silon,1,p);%rand(1,P);%0.2;     %bta for acceleration de-acceleration co-efficent  %%NOV17
        rt=0.02;%gamrnd(silon,0.02/silon,1,p);%rand(1,P);%0.3;       %constant3 reaction time of the driver  %%NOV17
