...
Acc = [];    %%% collecting all acceleration updates
X = [];
...
%     rt=abs(0.3*(1+silon(kl)*randn(1,P)));%rand(1,P);%0.3;       %constant3 reaction time of the driver
    alpha=gamrnd(silon,0.02/silon,1,P);%rand(1,P);%0.2;  %constant1 for friction coefficent (vehicle to the road);
    bta=gamrnd(silon,0.2/silon,1,P);%rand(1,P);%0.2;     %bta for acceleration de-acceleration co-efficent
    rt=gamrnd(silon,0.02/silon,1,P);%rand(1,P);%0.3;       %constant3 reaction time of the driver
%    alp(kl,:) = alpha;
%    bet(kl,:) = bta;
%    ret(kl,:) = rt;
