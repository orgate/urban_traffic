function alpha = powerlaw_exponent_v2(batch)
...
if(length(tempwt)>0)
    if(mod(batch-1,5)==0)
        [alpha, xmin, L] = plfit(tempwt,'nosmall');%fixing the xmin to be 3.0
...
save(['power_law_exponents_v2_batch',num2str(batch),'.mat'],'alpha','xmin','L');
