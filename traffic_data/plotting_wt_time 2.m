...
load Wait_time_bombay_bang_delhi12.mat % data of january, 2013
% x=[];y=[];z=[];
% xa=[];ya=[];za=[];
% x2=[];y2=[];z2=[];
% x2a=[];y2a=[];z2a=[];
...
for i=1:length(weekdays)   %%%%%changed from 30 for wt_time; it's 21 for ac_time
    for j=1:40000
        d = weekdays(1,i);
%         x=[x,wt{1,d}]; %Bombay
%         y=[y,wt{2,d}]; %Bangalore
%         z=[z,wt{3,d}]; %Delhi
%         xa=[xa,wta{1,d}]; %Bombay
%         ya=[ya,wta{2,d}]; %Bangalore
%         za=[za,wta{3,d}]; %Delhi
...
% following code is for the inset in "fig1e" (according to paper)
[yalensort, yaleni] = sort(yalen,'descend');
alpy_v = [];

for i=1:6122    % only 6122 taxis have at least one data point in Bangalore (assuming effective velocity threshold < 10kmph)
    i
    ys = ya{1,yaleni(1,i)};
    ys = ys(ys~=0);
    [alp, ymin, l] = plfit(ys);
