...
load Wait_time_no_taxis_bang08_v3.mat;
% meanvel = zeros(22,96);
% for ii=1:22
% for jj=1:96
% meanvel(ii,jj) = mean([mean_vel(1,ii,3*jj),mean_vel(1,ii,3*jj-1),mean_vel(1,ii,3*jj-2)]);
% end
% end
% %vel_mean = mean(meanvel,1);
% %meanvel(5,24) = mean([mean_vel(2,5,3*24),mean_vel(2,5,3*24-2)]);%due to
% %some data issues for JAN
% meanvel(14,43) = mean([mean_vel(2,14,3*43-2)]);%due to some data issues for APR
% meanvel(14,44) = mean([mean_vel(2,14,3*44),mean_vel(2,14,3*44-1)]);%due to some data issues for APR
% vel_mean = mean(meanvel,1);
...
for ii=1:D
for jj=1:96
taxi1(ii,jj) = mean([taxis_day(3,ii,3*jj),taxis_day(3,ii,3*jj-1),taxis_day(3,ii,3*jj-2)]);
end
end
