...
month = '04';%considering only January month of 2013
...
for l=1:D
    l1 = weekdays(l)
        ...
        for i=1:3 
            ...
            for tss=1:length(cty_tx_ind{1,i})
                ts = cty_tx_ind{1,i}(tss,1);
                ts_ind = floor(mod(time_stamp(ts),86400)/300)+1;
                taxis{i,l,ts_ind} = [taxis{i,l,ts_ind},m1(ts,1)];
                vel{i,l,ts_ind} = [vel{i,l,ts_ind},speed_col(ts,1)];
            end
            ...
            if(0)%(no_tx>0)   %check whether there is atleast one taxi present
%                wait{i,l} = cell(1,no_tx);
                ...
                for j=1:no_tx
                    ...
                    if(length(ind)>60)
                        ...
                        v1a=find(avg_kph<10);%intersect(find(avg_kph<10),find(avg_kph>0));%indices of where the avg velocity is less than 10kphr
                        %v1=find((avg_kph<5));
                        ...
                        v2a=find(speed<10);%intersect(find(speed<10),find(speed>0));%indices of where the avg velocity is less than 10kphr
                        %v1=find((avg_kph<5));
...
save(['Wait_time4_taxis_vel_bombay_bang_delhi',month,'.mat'],'taxis','mean_vel','median_vel');
