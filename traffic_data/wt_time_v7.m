...
function w = wt_time_v7(month,days);
...
%                wait{i,l} = cell(1,no_tx);
                ...
                for j=1:no_tx
                    ...
                    if(length(ind)>60)
                        ...
                        v9a=find(avg_kph<2.25);%intersect(find(avg_kph<5),find(avg_kph>0));%indices of where the avg velocity is less than 5kphr
                        %v1=find((avg_kph<5));
                        ...
                        v9b=find(avg_kph<2.25);%intersect(find(avg_kph<5),find(avg_kph>0));%indices of where the avg velocity is less than 5kphr
                        %v1=find((avg_kph<5));
 ...
 w=D;
save(['Waiting_time_taxis_half_avg_below_2_5_bombay_bang_delhi_fullyear2013',month,'_v2.mat']);
